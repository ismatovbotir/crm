<form id="quote-edit-form" wire:submit="save" class="h-full flex flex-col">

    {{-- ═══════════════════════════════════════════════════════════════════
         Предупреждение: статус будет сброшен
    ════════════════════════════════════════════════════════════════════ --}}
    @if(in_array($quote->status, ['sent', 'viewed']))
    <div class="flex-shrink-0 mb-3 flex items-start gap-2 rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-800">
        ...
        <span>КП в статусе <strong>«{{ $quote->status === 'sent' ? 'Отправлен' : 'Просмотрен' }}»</strong> — после сохранения будет переведено в <strong>«Черновик»</strong> и потребует повторной отправки клиенту.</span>
    </div>
    @endif
