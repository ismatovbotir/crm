...
<div class="flex items-center gap-1 ml-4">
    ...
    <div class="relative" x-data="{ open: false }">
        <button @click="open = !open"
            ...
            <span class="hidden md:block text-sm font-medium text-gray-700">{{ auth()->user()->name ?? 'Гость' }}</span>
        ...
        <div x-show="open"
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="w-full text-left px-4 py-2 text-sm text-danger-600 hover:bg-gray-50">
                    Выйти
                </button>
            </form>
        </div>
