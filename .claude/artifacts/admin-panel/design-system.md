...
**Поведение**:
- На `< lg`: сайдбар скрывается, открывается через гамбургер (off-canvas drawer, всегда полной ширины 16rem)
- На `>= lg`: сайдбар по умолчанию фиксированный 16rem (`w-64`), но **сворачиваем/разворачивается** кнопкой в хедере (иконка chevron-double, разворачивается на 180° при сворачивании) в узкий icon-only режим `w-20` (`lg:w-20`)
- Состояние collapsed/expanded хранится в Alpine-стейте на `<body x-data>` (`sidebarCollapsed`) и персистится через `localStorage` (`rsg-admin-sidebar-collapsed`), переживает переход между страницами
- В collapsed-режиме: текстовые лейблы пунктов меню, заголовки секций и текст в логотипе/карточке пользователя скрываются через `:class="sidebarCollapsed ? 'lg:hidden' : ''"`, каждый пункт меню сохраняет `title="..."` атрибут как нативный tooltip; секции меню разделяются тонким разделителем вместо текстового заголовка
- Контент-обёртка (`lg:pl-64` / `lg:pl-20`) и сам сайдбар анимируются через `transition-all duration-200`
