...
## 2026-07-08 — Семантическая цветовая палитра определена впервые + паттерн `.admin-theme` для brand-скоупинга (admin vs portal)
**Агенты**: pm-b2b-crm, admin-bi-developer, qa-tester
**Контекст**: задача "применить фирменный стиль RSG (#D00000) к admin-панели, портал не трогать" вскрыла, что семантические классы (`bg-primary-600`, `text-success-700`, `bg-warning-100`, `bg-danger-600` и т.п.), уже используемые примерно в 70 blade-файлах (admin И portal), физически не генерировали CSS — `resources/css/app.css` (Tailwind v4, единственный источник стилей и для admin, и для portal, и для guest/login через общий `@vite(['resources/css/app.css', ...])`) содержал только `--font-sans` в `@theme`, без единого `--color-*` custom property. Токены `primary/success/warning/danger` не имеют дефолтных значений в стандартной палитре Tailwind v4.
**Решение**:
1. `@theme` в `resources/css/app.css` теперь определяет полную шкалу (50-900) для `primary`, `success`, `warning`, `danger`, `info` — это глобальный дефолт, наследуемый всеми layout'ами (admin/portal/guest). Дефолт для `primary` — нейтральный Tailwind blue (НЕ бренд), чтобы не менять портал по сути задачи.
2. Бренд-красный (`#D00000`) применён только к `primary` и только скоуплен в CSS-классе `.admin-theme` (переопределяет `--color-primary-*` через каскад CSS custom properties), который вешается ТОЛЬКО на `<html>` в `resources/views/layouts/admin.blade.php`. `layouts/portal.blade.php` и `layouts/guest.blade.php` этот класс не получают и остаются на нейтральном дефолте.
3. `success/warning/danger/info` остаются семантически нейтральными везде, включая внутри `.admin-theme` — бренд-красный (`primary`, accent/навигация) не подменяет "danger" (обычный tailwind red, ошибка/просрочка), это разные узлы шкалы, различимые визуально.
**Причина**: единственный `app.css` на все контуры не позволяет физически разделить темы иначе, кроме как через CSS-каскад, скоупленный класс-селектором на layout-обёртке — стандартный паттерн multi-brand theming в Tailwind v4 (utility-классы ссылаются на `var(--color-primary-600)`, а не инлайнят литерал — подтверждено грепом собранного `public/build/assets/app-*.css`).
**Trade-off**: любой будущий agent, добавляющий новый layout/контур (например, будущий "партнёрский портал" или white-label), должен явно решить, наследует ли он `.admin-theme` или нейтральный дефолт — это не заложено автоматически. Chart.js-конфиги в `dashboard.blade.php`/`reports/index.blade.php` используют hardcoded hex (`#3B82F6`), НЕ читают CSS custom properties (JS не может напрямую резолвить Tailwind токены без доп. кода) — визуально не подхватят бренд-красный без отдельного фикса, если это когда-нибудь понадобится.
**См. также**: `.claude/shared/changelog.md` записи `[admin-bi-developer]`/`[qa-tester]` за 2026-07-08.
