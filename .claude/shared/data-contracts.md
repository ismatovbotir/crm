...
### `activity_log` (через `spatie/laravel-activitylog` — установлено 2026-07-06)

> ⚠️ Реальное имя таблицы пакета — `activity_log` (единственное число), **не** `audit_logs`. Миграция: `database/migrations/2026_07_06_150311_create_activity_log_table.php` (опубликована из пакета, стандартный стаб v5.0.0 — не редактируем вручную).

| Поле | Тип | Null | Default | Примечание |
|------|-----|------|---------|------------|
| id | bigint | no | auto | PK |
| log_name | string | yes | null | индекс. `useLogName()` в модели: `customer` / `lead` / `quote` / `invoice` |
| description | text | no | — | человекочитаемое описание события (updated/created/deleted) |
| subject_type | string | yes | null | морф-тип (класс модели), часть `nullableMorphs('subject')` |
| subject_id | bigint unsigned | yes | null | морф-id |
| event | string | yes | null | created / updated / deleted |
| causer_type | string | yes | null | морф-тип автора изменения (обычно `App\Models\User`) |
| causer_id | bigint unsigned | yes | null | морф-id автора |
| attribute_changes | json | yes | null | diff изменённых атрибутов (v5.0.0 — заменил `properties['attributes'/'old']` из более старых версий) |
| properties | json | yes | null | доп. метаданные события |
| timestamps | | | | |

**Индексы**: `log_name`, морф-индексы `subject_type+subject_id`, `causer_type+causer_id`
**Конфиг**: `config/activitylog.php` (опубликован)
**Подключено к моделям** (трейт `Spatie\Activitylog\Models\Concerns\LogsActivity` + опции `Spatie\Activitylog\Support\LogOptions` — в установленной версии 5.0.0 пакет переехал с `Spatie\Activitylog\Traits\LogsActivity`/`Spatie\Activitylog\LogOptions` (v4.x) на эти новые namespace; `getActivitylogOptions()` с `logOnlyDirty()` + `logOnly([...])` бизнес-полей, без служебных timestamps):
- `App\Models\Customer\Customer` — `name, legal_name, inn, business_type_id, segment, status, credit_limit, payment_terms_days, bank_id, bank_account`
- `App\Models\Lead\Lead` — `name, company, customer_id, status, score, budget, manager_id, source_id, converted_at, won_amount, lost_reason`
- `App\Models\Quote\Quote` — `number, customer_id, manager_id, status, currency, total, valid_until, sent_at, accepted_at, rejected_at`
- `App\Models\Invoice\Invoice` — `number, quote_id, customer_id, manager_id, status, shipment_status, due_date, total, paid_amount, sent_at`

Остальные модели (Ticket, Product, EquipmentRequest и т.д.) пока **не** логируются — не в скоупе этой итерации.
