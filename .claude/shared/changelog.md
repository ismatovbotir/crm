...
## 2026-07-07 — [claude-md-maintainer] Исправлен документационный дрейф Phase 9 (Leads kanban) + добавлена Roadmap Phase 14 (Leads Kanban реальная реализация + чистка leads.export)
- Часть 1.7 Roadmap, запись `Phase 9: UI Polish` (2026-05-05): удалены 2 пункта про "Leads kanban" (фикс скролла/`max-height`, фикс toggle) — аудит модуля Leads от 2026-07-07 грепом по коду подтвердил, что Kanban физически не существовал на момент этой фазы (`Index.php`/Blade были чисто табличными); пункты ошибочно документировали правки к несуществующему функционалу. Добавлено примечание с explicit-ссылкой на Phase 14, где Kanban реализован фактически впервые.
- Добавлена `### Phase 14: Leads Kanban (реальная реализация) + чистка мёртвого CSV-экспорта ✅ (Завершено — 2026-07-07)` после Phase 13 (последняя существовавшая на момент правки запись — исходное задание ожидало "Phase 11", но документ уже дошёл до Phase 13, поэтому пронумеровано как Phase 14): удаление мёртвого кода `admin.export.leads`/`ExportController::leads()`/permission `leads.export` (Blade-кнопка была убрана ранее, backend осиротел); Leads Kanban с нуля — `viewMode`/`scopedQuery()`/`kanbanColumns()`/`moveLeadStatus()` в `app/Livewire/Admin/Leads/Index.php`, drag-and-drop Blade с localStorage-персистом; 9 новых тестов `tests/Feature/Leads/LeadKanbanTest.php`; `php artisan test` → 193 passed (было 165).
- Источник: отчёты `[laravel-fullstack]`/`[admin-bi-developer]`/`[qa-tester]` за 2026-07-07 и прямой аудит модуля Leads в этой сессии (грепом подтверждено отсутствие Kanban до этой фазы).

## 2026-07-07 — [claude-md-maintainer] Roadmap Phase 13 (/admin/settings/roles) + уточнение §2.4
- Часть 1.7 Roadmap: добавлена `### Phase 13: Roles Management UI (/admin/settings/roles) ✅ (Завершено — 2026-07-07)` после Phase 12 — новая страница только для `super-admin` (`App\Livewire\Admin\Settings\Roles`, роут в существующей группе `role:super-admin`, а не через permission-гейт — обоснование приведено), locked-роли (`super-admin`/`client-admin`/`client-user`), переход `RolesSeeder` на аддитивную семантику (`givePermissionTo()` для уже существующих ролей вместо разрушительного `syncPermissions()`), 19 новых тестов, `php artisan test` 184 passed (было 165).
- §2.4 "Источник истины для прав: config/permissions.php": строка про "UI управления ролями (/admin/settings/roles)" дополнена — раньше это было чисто аспирационное упоминание (страницы не существовало в коде), теперь она реализована; добавлена короткая ремарка про gating по роли (не по permission) и ссылка на Roadmap Phase 13 за деталями (locked-роли, аддитивный сидер).
- Источник: отчёты `[laravel-fullstack]`, `[admin-bi-developer]`, `[qa-tester]` за 2026-07-07 (см. записи ниже/выше в этом файле).

## 2026-07-07 — [laravel-fullstack] Новая страница /admin/settings/roles: управление permissions ролей (super-admin only)

- Новый Livewire-компонент `app/Livewire/Admin/Settings/Roles.php`: список всех ролей из БД (`Role::query()->orderBy('name')->get()`, `#[Computed] roles()`), сгруппированный чеклист permissions по группам из `config('permissions.permissions')` (`#[Computed] permissionGroups()`). Public property `array $selectedPermissions` — состояние чекбоксов вида `['<role-name>' => ['<permission.key>', ...]]`, биндится в Blade стандартным Livewire-паттерном "несколько checkbox с одним `wire:model` + разные `value`". Экшен `savePermissions(string $roleName): void` делает `$role->syncPermissions(...)` с белым списком (`array_intersect` с полным списком permission-ключей из конфига — защита от произвольных строк в payload).
- **Locked-роли** (`private const LOCKED_ROLES = ['super-admin', 'client-admin', 'client-user']`): показываются в списке для прозрачности, но не редактируются. Guard — server-side, на уровне метода `savePermissions()` (`isLocked()` + `abort(403)`), не полагается только на то, что Blade скроет чекбоксы/кнопку. `super-admin` — wildcard-роль, остаётся `['*']` навсегда. `client-admin`/`client-user` — доступ ownership-based (портал), не через module permissions (уже описано комментарием в `config/permissions.php`).
- `mount()` компонента: `abort_unless(auth()->user()?->hasRole('super-admin'), 403)` — гейтинг НАМЕРЕННО по роли, а не по permission-строке `settings.roles` (которую эта же страница редактирует) — иначе self-lockout/privilege-escalation footgun. Уровень 1 (route middleware `role:super-admin`) — основная защита, это доп. defense-in-depth.
- Новый тонкий контроллер `app/Http/Controllers/Admin/Settings/RoleController.php` (`index()` → `view('admin.settings.roles')`, зеркалирует `UserController.php`).
- `routes/web.php`: `use App\Http\Controllers\Admin\Settings\RoleController;` + `Route::get('/roles', [RoleController::class, 'index'])->name('roles')` добавлен в уже существующую группу `Route::middleware('role:super-admin')->prefix('settings')->name('settings.')` (та же группа, что и `/users`) — новой middleware-группы не создавал.
- **Blade-файлы не создавал** (`resources/views/admin/settings/roles.blade.php`, `resources/views/livewire/admin/settings/roles.blade.php`) — это зона `admin-bi-developer`, следующий шаг. Роут временно вернёт `ViewNotFoundException` до тех пор, пока Blade не будет добавлен.
- **Архитектурное решение**, зафиксировано также в `.claude/shared/decisions-log.md` (PM): `database/seeders/RolesSeeder.php` — переход с разрушительного `syncPermissions()` на условно-аддитивный `givePermissionTo()` для ролей, которые уже существовали до текущего запуска сидера (`Role::firstOrCreate(...)->wasRecentlyCreated === false`). Для реально новых ролей (`wasRecentlyCreated === true`) поведение не изменилось — `syncPermissions()`, точный snapshot конфига. Trade-off: `config/permissions.php` остаётся гарантированным **минимумом** для каждой роли (сидер никогда не даст роли забыть permission из конфига), но обычный `migrate:fresh --seed` больше не стирает ручные правки, сделанные через новую UI `/admin/settings/roles`. Обратная сторона: если permission позже **удаляется** из конфига для роли, сидер больше не отзовёт его у тех, кто уже успел его получить в БД (осознанный компромисс, описан в докблоке класса).
- `php artisan migrate:fresh --seed`: прошёл end-to-end без ошибок (все 8 ролей созданы как новые в свежей БД → `syncPermissions()` ветка сработала штатно, как и раньше).
- `php artisan test`: **165 passed**, 385 assertions, 0 failed — регрессий не внесено.

## 2026-07-07 — [admin-bi-developer] Leads: убран экспорт CSV, таблица получила фиксированную высоту со sticky-заголовком

- `resources/views/livewire/admin/leads/index.blade.php`: удалена кнопка `@can('leads.export') <a href="{{ route('admin.export.leads') }}">...CSV</a> @endcan` из шапки страницы — по прямому запросу пользователя. Проверено grep'ом по всему проекту: это была единственная Blade-ссылка на `route('admin.export.leads')` и на permission `leads.export`. Сам роут (`routes/web.php:104`), `ExportController::leads()` и permission `leads.export` в `config/permissions.php` не трогал — это вне моей зоны (Blade/UI only); теперь этот backend-код, вероятно, осиротел и годится для отдельной чистки со стороны `laravel-fullstack`, если PM решит, что она нужна.
- Обёртка таблицы (`overflow-x-auto` → `overflow-auto` + `style="height: calc(100vh - 26rem); min-height: 20rem;"`) теперь имеет фиксированную высоту независимо от количества строк на странице (2 строки vs 50 занимают одинаковое место, лишнее скроллится внутри `<tbody>`). `<thead>` сделан `sticky top-0 z-10` с явным непрозрачным `bg-gray-50` на каждой `<th>` (заменил унаследованный `bg-gray-50/60` на `<tr>`, чтобы не было просвечивания строк под sticky-шапкой при скролле). Футер пагинации ниже `<x-card>` не тронут и продолжает работать как раньше. Конвенция `calc(100vh - Npx/rem)` для скролл-панелей подсмотрена в `resources/views/livewire/admin/invoices/create-form.blade.php` (там `calc(100vh - 18rem)` для двухколоночной формы) — здесь взял больший отступ (26rem), так как на странице лидов сверху ещё есть заголовок страницы и карточка фильтров, а снизу — футер пагинации внутри той же карточки.
- Не трогал `app/Livewire/Admin/Leads/Index.php` и другие Leads-компоненты (CreateForm/EditForm/Show) — задача чисто Blade/CSS.

## 2026-07-07 — [admin-bi-developer] Fix: 500-ошибка на /admin/settings/users из-за обращения к несуществующему ключу `label` в Blade

- Баг (найден `qa-tester`, независим от предыдущего фикса `laravel-fullstack` в этом же файле): `resources/views/livewire/admin/settings/users.blade.php:82` строил `$roleLabels = collect(config('permissions.roles'))->mapWithKeys(fn($v, $k) => [$k => $v['label']])` — но записи `config('permissions.roles')` содержат только ключи `guard` и `permissions`, `label` там никогда не было. `Undefined array key "label"` кидался внутри `mapWithKeys` до того, как отрабатывал любой `?? fallback` на строке использования — страница отдавала 500 для каждой строки с назначенной ролью (то есть практически всегда).
- Фикс: убрал локальный сломанный маппинг, строка `{{ $roleLabels[$role] ?? $role }}` теперь использует уже существующий computed `$this->availableRoles[$role] ?? $role` (тот же источник меток, что и в фильтре списка и в select формы создания/редактирования — единообразие). PHP-компонент (`app/Livewire/Admin/Settings/Users.php`) не трогал — его `availableRoles()`/`ROLE_LABELS` уже были корректны.
- `php artisan test`: **145 passed**, 0 failed (было 145 до задачи — состав тестов не менялся, просто из красного в зелёное). В частности `tests/Feature/Access/AdminUsersScopeTest.php::test_admin_settings_users_page_currently_errors_due_to_blade_bug` — вопреки ожиданию из постановки задачи, тест **не нужно адаптировать**: он с самого начала написан через `assertOk()` (ожидает успешный рендер), а не через ожидание ошибки/500 — то есть до фикса он был красным (страница реально падала), а после фикса стал зелёным, как и задумано автором теста. Адаптация `qa-tester` не требуется.
- Изменён только Blade-файл, вёрстка/дизайн/классы не трогал.

## 2026-07-07 — [laravel-fullstack] Fix: /admin/settings/users показывал клиентские portal-аккаунты и ломал role-dropdown
- Баг: `Users::users()` (`app/Livewire/Admin/Settings/Users.php`) тянул `User::query()` без фильтра по роли — в список внутренних сотрудников попадали и client-admin/client-user (per-company portal аккаунты, управляемые отдельно через `Customers\Show::attachUser()`/`detachUser()`). `Users::availableRoles()` читал весь `config('permissions.roles')` (8 ролей, включая клиентские) и обращался к несуществующим ключам `label`/`description` (реальная структура роли в конфиге — только `guard` + `permissions`), из-за чего в дропдауне отображалось " — " без текста.
- Устранено дублирование списка внутренних ролей: добавлена константа `User::INTERNAL_ROLES` (`app/Models/User.php`) — единый источник правды вместо двух литеральных массивов. `User::scopeManagers()` и `App\Helpers\Acl::isInternal()` (`app/Helpers/Acl.php`) теперь используют эту константу; поведение обоих методов не изменилось.
- `Users::users()` теперь строится через `User::query()->managers()->with('roles')->...` — client-admin/client-user больше никогда не попадают в список и в фильтр.
- `Users::availableRoles()` фильтрует `config('permissions.roles')` через `array_intersect_key(..., array_flip(User::INTERNAL_ROLES))` — dropdown фильтра списка и dropdown role-select в форме создания/редактирования показывают только 6 внутренних ролей. Добавлена приватная карта `Users::ROLE_LABELS` (человекочитаемые русские названия ролей) с фоллбэком на `Str::headline($slug)` — чинит пустые " — " в дропдауне.
- Проверено: `openCreate()` делает `$this->reset(['role', ...])` → `role` становится `''` (не дефолтится ни на какую роль, в т.ч. клиентскую) — валидация `role.required` отработает как раньше, изменений не потребовалось.
- Побочное наблюдение (не в скоупе, не тронуто): `resources/views/livewire/admin/settings/users.blade.php:82` строит `$roleLabels` тем же несуществующим `$v['label']`, но там есть fallback `?? $role` — просто показывает slug вместо красивого имени, не ломается. Правка — за `admin-bi-developer`.
- Изменённые файлы: `app/Models/User.php`, `app/Helpers/Acl.php`, `app/Livewire/Admin/Settings/Users.php`.
- Не менялись: `config/permissions.php`, Blade/сайдбар, тикет-система.
- `php artisan test`: 132/132 passed, 273 assertions — baseline не сломан.

## 2026-07-07 — [qa-tester] Тесты на scoping-фикс /admin/settings/users + найден блокирующий Blade-баг
- Добавлен `tests/Feature/Access/AdminUsersScopeTest.php` (13 тестов), покрывает фикс `laravel-fullstack` по `Users::users()`/`availableRoles()`:
  - `users()`/`scopeManagers()` никогда не включает `client-admin`/`client-user`, даже если такие пользователи есть в БД — только 6 внутренних ролей; проверено на полном наборе ролей + пустой список, если в БД только клиентские аккаунты; поиск (`search`) сохраняет фильтр.
  - `availableRoles()` никогда не предлагает клиентские роли (ни в фильтре списка, ни в форме) — ключи строго равны `User::INTERNAL_ROLES`, у каждой роли непустой человекочитаемый label.
  - Happy-path CRUD не сломан рефакторингом: создание internal-пользователя с internal-ролью, редактирование (смена имени/email/роли, пустой пароль не затирает старый), `toggleActive()`.
  - Найдена и задокументирована (не блокер, не в скоупе фикса) "known gap": `Users::rules()` валидирует `role` как `required|string`, не `in:` — прямая запись `$component->role = 'client-user'` перед `save()` не блокируется валидацией. Тест `test_save_does_not_reject_a_client_role_assigned_via_direct_property_write` документирует текущее (permissive) поведение, ничего не фиксит. Возможный фикс (`Rule::in(User::INTERNAL_ROLES)` в `Users::rules()`) — за `laravel-fullstack`, если посчитают нужным ужесточить.
  - Роут-уровень: guest → redirect на `/login`, internal-роль без `super-admin` → 403, `client-user` → 403 на `/admin/settings/users`. Регрессий в существующем `RouteSmokeTest`/`AccessControlTest` нет (роуты не менялись).
- **НАЙДЕН БАГ (не мой скоуп, передаю `admin-bi-developer`)**: `resources/views/livewire/admin/settings/users.blade.php:82` (`$roleLabels = collect(config('permissions.roles'))->mapWithKeys(fn($v,$k) => [$k => $v['label']])`) кидает `ErrorException: Undefined array key "label"` и **реально роняет страницу в 500** для любого запроса, где хотя бы одна строка таблицы имеет назначенную роль — то есть практически всегда, включая случай, когда единственный пользователь в списке — сам авторизованный super-admin. Это уточняет запись `[laravel-fullstack]` от сегодня выше ("...есть fallback `?? $role` — просто показывает slug, не ломается") — **это неверно**: исключение бросается при построении `$roleLabels` (внутри `mapWithKeys`), т.е. до того как fallback `??` вообще успевает сработать. Баг предсуществующий (с первого коммита `c27788f` этого файла), не связан с сегодняшним scoping-фиксом. Воспроизведение: `GET /admin/settings/users` авторизованным super-admin'ом при наличии хотя бы одного пользователя с ролью в списке (RefreshDatabase + сидированные роли + `actingAs(superAdmin)` уже достаточно, т.к. сам super-admin — тоже строка списка). Задокументировано падающим тестом `test_admin_settings_users_page_currently_errors_due_to_blade_bug` (намеренно не переписан "под баг" — см. QA policy). Предлагаемый фикс (для `admin-bi-developer`): использовать уже корректный `$this->availableRoles` (или publичный доступ к `Users::ROLE_LABELS`) вместо повторного вычисления сломанного маппинга в самом Blade.
- `php artisan test`: 145/145 запущено, 144 passed / 1 failed (намеренно, см. выше) — регрессий в остальных 144 не найдено, baseline (132/132 до этой сессии) не сломан.

## 2026-07-07 — [admin-bi-developer] Отдельный пункт "Пользователи" в сайдбаре после "Настройки"
- `resources/views/admin/partials/sidebar.blade.php`: добавлен новый ключ иконки `users` (heroicons-outline "group of people", отличный от `gear`)
- В блоке Settings (`@if(\App\Helpers\Acl::can('settings.users'))`) сразу после существующей ссылки "Настройки" добавлена вторая ссылка "Пользователи" — тот же URL `/admin/settings/users`, тот же permission `settings.users`, иконка `users` вместо `gear`
- Ссылка "Настройки" не изменена и не удалена
- Роуты, Livewire-компонент `Users.php` и `config/permissions.php` не затронуты
...
## 2026-07-07 — [laravel-fullstack] Fix: defense-in-depth — whitelist-валидация `role` в `Admin\Settings\Users`

- Баг (найден `qa-tester`, `tests/Feature/Access/AdminUsersScopeTest.php::test_save_does_not_reject_a_client_role_assigned_via_direct_property_write`): `Users::rules()` валидировал поле `role` как `required|string` — без проверки против whitelist `User::INTERNAL_ROLES`. Через обычный UI это не эксплуатировалось (`availableRoles()` и так предлагает только внутренние роли), но ничто на сервере не мешало принять `client-admin`/`client-user`, если `$role` выставлен любым другим путём (tampered Livewire payload и т.п.).
- Фикс: `app/Livewire/Admin/Settings/Users.php` — `'role'` теперь `['required', 'string', Rule::in(User::INTERNAL_ROLES)]` (добавлен импорт `Illuminate\Validation\Rule`); добавлено сообщение `'role.in' => 'Недопустимая роль.'` в `messages()`. Больше ничего в файле не менялось (openCreate/openEdit/save/toggleActive/availableRoles — без изменений).
- `php artisan test`: **144 passed, 1 failed** (было 145 passed, 0 failed). Упавший тест — `AdminUsersScopeTest::test_save_does_not_reject_a_client_role_assigned_via_direct_property_write` — это тот самый тест, который документировал "known gap" и явно писал, что не проверяет требование, а лишь фиксирует permissive-поведение (см. комментарий над тестом в файле, строки 225-237, с прямой ссылкой на `Rule::in(User::INTERNAL_ROLES)` как вероятный фикс). После фикса поведение теста больше не соответствует его названию/ассертам (`hasRole('client-user')` теперь false, т.к. `save()` бросает `ValidationException`) — тест нужно адаптировать/удалить силами `qa-tester`, сам файл теста не трогал (не моя зона).

## 2026-07-07 — [qa-tester] Адаптация теста под закрытый defense-in-depth gap в `Admin\Settings\Users`

- `tests/Feature/Access/AdminUsersScopeTest.php` — заменил `test_save_does_not_reject_a_client_role_assigned_via_direct_property_write` (документировал permissive-поведение как "known gap") на `test_save_rejects_a_client_role_assigned_via_direct_property_write` (строки 224-265): теперь тест утверждает, что `save()` кидает `ValidationException` при прямой записи `$component->role = 'client-admin'|'client-user'` и НЕ создаёт пользователя (`assertDatabaseMissing('users', ['email' => ...])` в `finally`-блоке после `expectException`). Покрыты оба клиентских значения роли через `#[DataProvider('clientRoleProvider')]` (метод `clientRoleProvider()`, строки 259-265) — PHPUnit 12 в проекте требует атрибут `#[DataProvider]`, а не устаревшую докблок-аннотацию `@dataProvider` (аннотация тихо не подхватывалась, тест падал с `Too few arguments`).
- Докблок над тестом (строки 226-235) переписан точечно: снята пометка "KNOWN GAP, not fixed here" — теперь описывает закрытую defense-in-depth проверку (`Rule::in(User::INTERNAL_ROLES)` уже применена в `Users::rules()` силами `laravel-fullstack`). Секционный комментарий-разделитель (было `// ── Known gap (не блокер — документировано по запросу QA) ───`) переименован в `// ── Role validation (defense-in-depth, closed) ───`. Верхний классовый докблок (строки 10-49) на "known gap" не ссылался — правка не потребовалась.
- Продовый код (`app/Livewire/Admin/Settings/Users.php`) не трогал — фикс уже сделан `laravel-fullstack`.
- Итог прогона `php artisan test`: **146 passed**, 0 failed, 313 assertions (было 144 passed/1 failed до правки; было бы 145/0 без учёта заменённого теста — итоговые 146 вместо 145, т.к. один тест раскрылся в два кейса через data provider).

## 2026-07-07 — [laravel-fullstack] Fix: 4 бага в модуле Leads (аудит sidebar #3)

Аудит по образцу недавнего Users-admin разбора нашёл и устранил 4 воспроизводимых бага:

- **Bug 1 — краш при создании/редактировании лида без явного менеджера**: `leads.manager_id` — `NOT NULL` на уровне БД (`constrained('users')->restrictOnDelete()`), но `CreateForm::rules()`/`EditForm::rules()` валидировали поле как `nullable`. Для sales-director/super-admin (у них нет авто-заполнения менеджера в `mount()`, оно только для `sales-manager`) пустой выбор в дропдауне "— не назначен —" проходил валидацию и падал необработанным `QueryException` при `Lead::create()`/`update()`. Фикс: `'manager_id' => 'required|exists:users,id'` в обоих компонентах (`app/Livewire/Admin/Leads/CreateForm.php`, `app/Livewire/Admin/Leads/EditForm.php`) + сообщение `'manager_id.required' => 'Выберите менеджера.'`. Авто-заполнение для `sales-manager` в `CreateForm::mount()` не трогал — оно и так удовлетворяет требованию для этой роли. Blade-разметку (наличие пустой опции в дропдауне) не трогал — не входило в описанный скоуп фикса.
- **Bug 2 — `won_amount` не сохранялся при редактировании won-лида**: `edit-form.blade.php` рендерил `wire:model="won_amount"` при `status === 'won'`, но `EditForm` не объявлял публичное свойство `$won_amount` — поле не биндилось и никогда не попадало ни в `fill()`, ни в `rules()`, ни в `$this->lead->update($data)`. Фикс (`app/Livewire/Admin/Leads/EditForm.php`): добавлено `public ?string $won_amount = null;`, добавлено в массив `fill($this->lead->only([...]))`, добавлено правило `'won_amount' => 'nullable|numeric|min:0'` — `save()` не менял (уже передаёт весь `$data` из `validate()` в `update()`, `won_amount` подхватился автоматически).
- **Bug 3 — конвертированные лиды (`status = 'client'`) нельзя было сохранить через Edit**: `Show::convertToCustomer()` выставляет `status = 'client'`, которого нет в whitelist `'status' => 'required|in:new,qualified,contacted,in_negotiation,won,lost'` в `CreateForm`/`EditForm` — при открытии Edit на конвертированном лиде и попытке Save (даже без изменений) валидация статуса всегда падала, лид становился недоступен для редактирования полей вроде телефона/заметок. Фикс: кнопка "Редактировать" в `resources/views/livewire/admin/leads/show.blade.php` теперь скрыта при `$lead->status === 'client'` (тот же паттерн, что уже использовался для кнопки "Конвертировать"); плюс defense-in-depth — `EditForm::mount()` (`app/Livewire/Admin/Leads/EditForm.php`) делает `abort_if($this->lead->status === 'client', 403, '...')`, закрывая прямой заход по URL модалки редактирования.
- **Bug 4 — конвертированные лиды не находились в списке/поиске**: `Index::baseQuery()` скрывал `status = 'client'` через `->when(! $this->statusFilter, fn ($q) => $q->where('status', '!=', 'client'))` — это исключение подавляло даже активный текстовый поиск (`$this->search`) по имени/телефону/email, и в дропдауне статусов не было пункта `client`, так что явно посмотреть на конвертированных лидов было невозможно. Фикс (`app/Livewire/Admin/Leads/Index.php`): условие теперь `->when(! $this->statusFilter && ! $search, ...)` — исключение действует только когда НЕТ ни фильтра, ни поискового запроса; в `$statuses` массив `render()` добавлен `'client'`; в `resources/views/livewire/admin/leads/index.blade.php` дропдаун статусов получил пункт `'client' => 'Конвертирован'` (badge-компонент `x-lead-status-badge` уже поддерживал `client` → "Клиент"/зелёный, менять не пришлось).
- `.claude/shared/data-contracts.md` (секция Leads) обновлена: `status` enum теперь документирует `client` как системное значение (не выбирается вручную); `manager_id` помечен как теперь-`required` и на уровне validation (не только БД); добавлены 2 новые строки-примечания про правило "нельзя редактировать конвертированный лид" и про то, что список больше не прячет `client` от активного поиска.
- Затронутые permissions: не менялись (используются существующие `leads.*`/`LeadPolicy`, которую явно просили не трогать).
- `php artisan test`: **146 passed**, 0 failed, 313 assertions (без изменений от базовой линии — регрессий не внесено; `tests/Feature/Leads/LeadCreationTest.php` зелёный).
- Для `qa-tester`: новых тестов на эти 4 бага пока нет — стоит добавить регрессионные кейсы (создание/редактирование лида без менеджера → ValidationException вместо QueryException; сохранение `won_amount` через EditForm; попытка Edit конвертированного лида → 403; поиск/фильтр находит `client`-лида).
- Вне скоупа, не тронуто (по явному указанию задачи): Kanban-вид (не существует в коде, несмотря на упоминания в CLAUDE.md/changelog — отдельный вопрос документационного дрейфа для PM/`claude-md-maintainer`), `LeadPolicy`, Ticket System/EquipmentRequest.

## 2026-07-07 — [qa-tester] Регрессионные Feature-тесты на модуль Leads (Index/EditForm/конвертация/Policy)

Закрыл пробелы в покрытии, описанные в предыдущей записи `[laravel-fullstack]` выше — до этого единственным тестом модуля был `LeadCreationTest.php`.

- `tests/Feature/Leads/LeadIndexTest.php` (7 тестов) — scoping `Index::baseQuery()`: `sales-director`/`super-admin` видят лиды всех менеджеров (базовый сценарий "sales-manager видит только своих" уже был покрыт в `AccessControlTest::test_leads_index_only_lists_own_leads_for_sales_manager`, не дублировал); поиск по имени/компании/телефону/email; регрессия Bug 4 — конвертированный (`status=client`) лид находится активным текстовым поиском по телефону; без поиска/фильтра `client`-лиды по умолчанию скрыты; явный `statusFilter=client` их показывает.
- `tests/Feature/Leads/LeadEditTest.php` (9 тестов) — регрессия Bug 1: `EditForm::save()` с `manager_id=null` даёт `assertHasErrors(['manager_id'])`, а не краш; регрессия Bug 2: `won_amount` персистится при `status=won`, остаётся `null` при обычном сохранении без изменений; регрессия Bug 3: `EditForm` кидает 403 и на `sales-manager`, и на `sales-director` при попытке открыть конвертированный (`status=client`) лид.
- `tests/Feature/Leads/LeadConversionTest.php` (3 теста) — полный happy-path `Show::convertToCustomer()`: создаётся `Customer` (name/business_type_id/status=active/customer_since=today), `Contact` (is_primary=true, имя/телефон/email лида), сам `Lead` обновляется (`customer_id`, `status=client`, `converted_at`), пишется `LeadActivity` типа `conversion`; повторная конвертация уже сконвертированного лида — 422 (`abort_unless`) без дублирования `Customer`; сторонний `sales-manager` не может открыть `Show`/сконвертировать чужой лид (403 на уровне `mount()`, т.к. `view()`/`update()` в `LeadPolicy` используют одинаковый ownership-чек — независимо изолировать их для этой роли невозможно, что задокументировано комментарием в тесте).
- `tests/Feature/Leads/LeadPolicyTest.php` (3 теста, юнит-стиль на `LeadPolicy` напрямую, без Livewire/HTTP) — `super-admin`/`sales-director` проходят `view`/`update`/`delete` на ЛЮБОМ лиде вне зависимости от `manager_id`; `sales-manager` проходит только на своих (false на чужих); `tech-support` (нет `leads.*` permissions в `config/permissions.php`) получает `false` на `viewAny`/`create`/`view`/`update`/`delete` даже если формально "владеет" лидом по `manager_id`.

**2 новых бага найдены** (не чинил, оставил тесты падающими намеренно — для `laravel-fullstack`):

- **Bug 5 — `EditForm::mount()` крашится TypeError'ом для любого лида без `lost_reason`**: `public string $lost_reason = '';` объявлено НЕ-nullable, но `leads.lost_reason` в БД `nullable` и по умолчанию `null` для абсолютного большинства лидов (никогда не помечались "lost"). `mount()` делает `$this->fill($this->lead->only([..., 'lost_reason', ...]))`, Livewire пытается присвоить `null` в типизированное `string`-свойство → `TypeError: Cannot assign null to property App\Livewire\Admin\Leads\EditForm::$lost_reason of type string`. На практике это ломает открытие Edit-формы почти для любого лида, кроме тех, что когда-либо были помечены `lost` с указанием причины. Тест: `tests/Feature/Leads/LeadEditTest.php::test_edit_form_mount_crashes_when_lead_has_no_lost_reason` (падает намеренно). Исправление: сделать свойство `public ?string $lost_reason = null;` (симметрично уже нетипизированному-nullable `won_amount` в этом же классе).
- **Bug 6 — `EditForm::save()` крашится при очистке `won_amount` до пустой строки**: правило `'won_amount' => 'nullable|numeric|min:0'` — Laravel считает `''` "пустым" под `nullable` и пропускает проверку `numeric`, так что `validate()` возвращает `won_amount => ''`. Но `Lead::$casts['won_amount'] = 'decimal:2'` не умеет кастовать пустую строку и `$this->lead->update($data)` падает с `"Unable to cast value to a decimal."`. Воспроизводится реалистично: пометить лид `won`, ввести сумму, затем очистить поле (`wire:model` отправляет `''`) и сохранить. Тест: `tests/Feature/Leads/LeadEditTest.php::test_clearing_won_amount_to_empty_string_crashes_on_save` (падает намеренно). Исправление: привести `''` к `null` перед `update()`, например `$data['won_amount'] = $data['won_amount'] === '' ? null : $data['won_amount'];` в `save()`.
- `.claude/shared/data-contracts.md` не трогал — Bug 5/6 не меняют схему/контракты, только исправление типов свойств Livewire-компонента; `laravel-fullstack` может обновить секцию Leads по факту фикса, если сочтёт нужным.
- `php artisan test` (полный прогон): **165 tests, 163 passed, 2 errors, 382 assertions**. 2 ошибки — это два намеренно оставленных падающих теста (Bug 5, Bug 6), задокументированных выше. Базовые 146 тестов (включая старый `LeadCreationTest.php`) остались зелёными — регрессий не внесено.
- Не трогал: `app/Livewire/Admin/Leads/*`, `app/Policies/LeadPolicy.php`, миграции, Blade-шаблоны, Ticket System/EquipmentRequest — согласно границам роли.

## 2026-07-07 — [laravel-fullstack] Leads: исправлены Bug 5 и Bug 6 из EditForm (крэш mount() без lost_reason, крэш save() при очистке won_amount/budget)

Фикс двух багов, найденных `qa-tester` в `tests/Feature/Leads/LeadEditTest.php` (см. запись выше) и намеренно оставленных падающими.

- `app/Livewire/Admin/Leads/EditForm.php`:
  - Bug 5: `public string $lost_reason = '';` → `public ?string $lost_reason = null;`. Устраняет `TypeError` в `mount()` при `fill()` из лида без `lost_reason` (подавляющее большинство лидов, никогда не помеченных `lost`). Валидация `'lost_reason' => 'nullable|string|max:255'` не менялась — уже корректна.
  - Bug 6: в `save()` перед `$this->lead->update($data)` добавлена коэрсия `''` → `null` для `won_amount`: `$data['won_amount'] = $data['won_amount'] === '' ? null : $data['won_amount'];`. Устраняет `"Unable to cast value to a decimal."` от `Lead::$casts['won_amount'] = 'decimal:2'`, когда пользователь очищает поле суммы у `won`-лида и сохраняет.
  - По той же причине (идентичный паттерн `'budget' => 'nullable|numeric|min:0'` + `'budget' => 'decimal:2'` в `Lead::$casts`) добавлена такая же коэрсия для `budget` в `EditForm::save()`, хотя отдельного теста на это не было — превентивно, чтобы не оставлять симметричный краш нетронутым.
- `app/Livewire/Admin/Leads/CreateForm.php`: та же коэрсия `''` → `null` для `budget` добавлена в `save()` (тот же риск, что и в EditForm, `won_amount`-поля в CreateForm нет).
- `php artisan test tests/Feature/Leads`: **23 passed**, 85 assertions — оба намеренно падающих теста теперь зелёные (`test_edit_form_mount_crashes_when_lead_has_no_lost_reason`, `test_clearing_won_amount_to_empty_string_crashes_on_save`).
- `php artisan test` (полный прогон): **165 passed**, 385 assertions, 0 failed — регрессий не внесено (было 163/165 passed до фикса).
- `.claude/shared/data-contracts.md` не трогал — изменения только в типах Livewire-свойств и pre-save коэрсии, схема БД/модели не менялась.
- Не трогал Blade-файлы (`admin-bi-developer` параллельно работает над `index.blade.php`), Kanban (не существует), другие модули Leads вне этих двух багов — согласно ограничению задачи.

## 2026-07-07 — [admin-bi-developer] Blade-вёрстка `/admin/settings/roles` (управление permissions ролей)

- Новый `resources/views/admin/settings/roles.blade.php` — тонкая обёртка (`@extends('layouts.admin')` + `<livewire:admin.settings.roles />`), зеркалирует `admin/settings/users.blade.php`.
- Новый `resources/views/livewire/admin/settings/roles.blade.php` — основной UI: список всех 8 ролей (`$this->roles`), каждая — раскрываемая карточка (`x-card` + Alpine `x-data="{ expanded }"`, локальный `x-show`/`x-transition`, без `x-collapse`-плагина — в проекте он не подключён). Locked-роли (`isLocked()`: `super-admin`, `client-admin`, `client-user`) открыты по умолчанию, показывают бейдж «Не редактируется», пояснительную подпись (своя для super-admin и общая для client-*) и read-only список текущих permissions роли (`$role->getPermissionNames()`, лейблы через `flatMap` по `$this->permissionGroups()` — не полагался на предположение о заполненности `$this->selectedPermissions` для locked-ролей, взял данные роли напрямую). Остальные 5 ролей — чеклист чекбоксов, сгруппированный по `$this->permissionGroups()` (человекочитаемые русские заголовки групп: Лиды/Клиенты/КП/Инвойсы/Продажи/Возвраты/Каталог/Тикеты/Заявки на оборудование/Отчёты/Настройки), биндинг `wire:model="selectedPermissions.{{ $role->name }}"` на каждый чекбокс, кнопка «Сохранить» → `wire:click="savePermissions('{{ $role->name }}')"` с `wire:loading`/`wire:target`-состоянием именно на эту кнопку (не глобальный лоадер). Flash `session('success')` в стандартном для проекта стиле (`bg-success-50 border-success-200 text-success-700`, как в `users.blade.php`).
- `resources/views/admin/partials/sidebar.blade.php`: добавлена новая иконка `shield` в массив `$icons`; в блоке Settings (после «Настройки»/«Пользователи») добавлен пункт «Роли и права» → `/admin/settings/roles`. Ссылка обёрнута явной проверкой `@if(auth()->user()->hasRole('super-admin'))` (а не permission-строкой `settings.roles` через стандартный `$navLink`-гейт) — по требованию задачи, чтобы UI не мог разойтись с реальным роут-гейтом (`role:super-admin` middleware), даже гипотетически.
- PHP-логику компонента (`app/Livewire/Admin/Settings/Roles.php`), роуты, контроллер, `RolesSeeder` не трогал — уже сделаны `laravel-fullstack`.
- Проверка: временный (не оставленный в репозитории) Feature-тест прогнал рендер страницы под всеми 8 ролями — под `super-admin` рендерится 200 OK с корректным HTML для каждой из 8 карточек (включая locked), под остальными 7 ролями — 403/redirect, как и ожидает `role:super-admin` middleware.
- `php artisan test`: **165 passed**, 385 assertions, 0 failed — регрессий не внесено (baseline подтверждён).

## 2026-07-07 — [laravel-fullstack] Чистка мёртвого кода CSV-экспорта лидов + backend для Kanban-доски Leads

**1. Удаление осиротевшего экспорта лидов** (Blade-кнопку убрал `admin-bi-developer` ранее этой же сессией):
- `routes/web.php`: удалена строка `Route::get('/export/leads', ...)->name('export.leads')` из блока «CSV Exports». `export.customers`/`export.invoices` не тронуты. Перед правкой перечитал файл заново (учитывая, что параллельно шла правка секции Settings/roles другим агентом) — конфликта не возникло, merge не потребовался.
- `app/Http/Controllers/Admin/ExportController.php`: удалён метод `leads()` целиком; удалён ставший неиспользуемым `use App\Models\Lead\Lead;`. Методы `customers()`/`invoices()` и их импорты (`Customer`, `Invoice`) не тронуты.
- `config/permissions.php`: удалена запись `'leads.export' => 'Экспорт лидов (CSV)'` из группы `permissions.leads`; удалена строка `'leads.export'` из списка permissions роли `sales-director` (сама роль и остальные её permissions не тронуты). `customers.export`/`invoices.export` — не тронуты нигде в файле.
- Повторный grep по всему репозиторию на `leads.export`/`export.leads`/`ExportController::` перед удалением подтвердил: совпадения были только в этих 3 файлах, тестов/Blade-ссылок не осталось.
- Не запускал `RolesSeeder` — permission `leads.export` в БД (если был выдан ранее через сидер/UI) сам по себе не отзывается этой правкой конфига (это ожидаемо и симметрично уже задокументированному в предыдущей записи `givePermissionTo()`-компромиссу); при следующем `migrate:fresh --seed` permission просто не будет создан заново, так как ушёл из конфига.

**2. Kanban-доска Leads — backend/Livewire часть** (`app/Livewire/Admin/Leads/Index.php`), Blade/Alpine/drag-and-drop — следующий шаг `admin-bi-developer`:
- `public string $viewMode = 'table';` — новое публичное свойство переключателя режима (НЕ в `$queryString`, персистентность между визитами — через localStorage на фронтенде, ключ предложен `rsg-admin-leads-view-mode`, по аналогии с `rsg-admin-sidebar-collapsed` в `layouts/admin.blade.php`; сам JS/Alpine — зона `admin-bi-developer`).
- `public function setViewMode(string $mode): void` — валидирует `in_array($mode, ['table', 'kanban'], true)`, иначе no-op.
- Рефакторинг: общая логика (search-фильтр + ownership-скоуп по `manager_id` для не-director/super-admin ролей) вынесена из `baseQuery()` в новый `protected function scopedQuery(): Builder`; `baseQuery()` (используется таблицей) теперь строится поверх `scopedQuery()` + добавляет `statusFilter`/client-exclusion — поведение таблицы не изменилось (100% та же логика, что была).
- `protected function kanbanColumns(): array` — строит 6 колонок (`new`, `qualified`, `contacted`, `in_negotiation`, `won`, `lost`; статусы вынесены в `protected const KANBAN_STATUSES`) поверх `scopedQuery()` (то есть тот же ownership-скоуп и тот же `search`, что и в таблице — отдельного незащищённого запроса нет). Для каждой колонки: `total` (полный count) и `leads` — не более `KANBAN_COLUMN_LIMIT = 50` записей (`->latest()->limit(50)->get()`), плюс `remaining = max(0, total - count)` для индикатора «и ещё N» на стороне Blade.
- **Статус `client` сознательно не получает колонку** — терминальный/системный статус, недоступный для ручного выбора нигде в проекте (`EditForm::rules()`, `Show::changeStatus()` whitelist) — Kanban следует этому же решению.
- **Решение по `statusFilter` в kanban-режиме: игнорируется.** В канбане статус и так виден через 6 колонок; применение `statusFilter` в этом режиме просто опустошило бы 5 из 6 колонок без пользы. `search`, наоборот, учитывается — консистентно с таблицей.
- `public function moveLeadStatus(int $leadId, string $status): void` — дословно повторяет защитный паттерн `Show::changeStatus()`: `Lead::findOrFail($leadId)` → `$this->authorize('update', $lead)` (тот же ownership-контроль `LeadPolicy::update()`, что и у таблицы/формы редактирования) → guard `if ($lead->status === 'client') return;` → whitelist по `KANBAN_STATUSES` (не входит — тихий no-op, без exception, чтобы неудачный drop просто визуально «не сработал») → если `$status === $lead->status` — no-op (без записи в activity) → иначе `update()` + `LeadActivity` (`type='status_change'`, `title='Изменён статус'`, `description="Статус изменён на: {$status}"`, `meta=['from'=>$oldStatus,'to'=>$status]`, `user_id=auth()->id()`) — идентично `Show::changeStatus()`, единая история timeline независимо от места смены статуса.
- `render()`: добавлены в view-данные `kanbanColumns` (пусто в table-режиме — колонки не считаются зря) и `kanbanStatuses` (порядок 6 статусов для Blade-цикла). Существующие `leads`/`statuses` для таблицы не тронуты.
- Blade (`resources/views/livewire/admin/leads/index.blade.php`) не трогал — по условию задачи, это зона `admin-bi-developer`.
- `php artisan test`: **165 passed**, 385 assertions, 0 failed — регрессий не внесено (kanban ещё не покрыт тестами, это следующий шаг `qa-tester`).

## 2026-07-07 — [qa-tester] Тестовое покрытие `/admin/settings/roles` (управление правами ролей)

Новый файл `tests/Feature/Access/RolePermissionsManagementTest.php` — 19 тестов на фичу `App\Livewire\Admin\Settings\Roles` (backend от `laravel-fullstack`, Blade от `admin-bi-developer`):

- **Route-level доступ** (только `super-admin`): `super-admin` → 200; `sales-director`, `sales-manager`, `tech-support`, `catalog-manager`, `accountant`, `client-admin`, `client-user` → 403 (data-provider, единый тест `test_non_super_admin_role_cannot_reach_settings_roles`); guest → redirect на `/login`. Клиентские роли получают 403 ещё на внешнем `role:super-admin|sales-director|...` middleware группы `/admin` (не доходят до внутреннего `role:super-admin` на `/admin/settings`) — поведение корректное, тест это не разделяет отдельно, т.к. итоговый код ответа один и тот же (403).
- **Toggle permission + персист**: добавление ранее отсутствующего у `sales-manager` права (`reports.sales`) через `Livewire::test(Roles::class)->set('selectedPermissions.sales-manager', [...])->call('savePermissions', 'sales-manager')` — проверено, что `Role::hasPermissionTo()` в БД отражает изменение, baseline-права не пострадали, и что **свежий** экземпляр компонента (новый `mount()`) подхватывает изменение из БД. Аналогичный тест на удаление существующего права (`leads.delete`).
- **Locked-роли (`super-admin`, `client-admin`, `client-user`) не редактируются** даже прямым вызовом `savePermissions()` с подставленным `selectedPermissions` — 3 теста, каждый проверяет `->assertForbidden()` (403 из `abort()` внутри метода) И что permissions роли в БД не изменились (wildcard-набор `super-admin` остался полным; `client-admin`/`client-user` остались с 0 permissions).
- **Read-only рендер локед-ролей**: `super-admin` видит все 8 ролей (включая `client-admin`/`client-user`) без ошибок рендера; отдельный unit-тест на `isLocked()` для всех 8 ролей.
- **Happy-path smoke**: рендер групп permissions с заголовками групп (Лиды/Клиенты/КП/Инвойсы/Тикеты/Настройки), без ошибок.
- **Регрессия на аддитивность `RolesSeeder`**: ручной `givePermissionTo('reports.sales')` роли `sales-manager` (симуляция правки через UI) переживает повторный прогон `RolesSeeder` — permission не потерян, и все baseline-права из `config/permissions.php` по-прежнему присутствуют. Отдельный тест подтверждает, что **первый** прогон сидера даёт роли ровно permissions из конфига, без лишнего (`syncPermissions` для новой роли).

**Результат**: `php artisan test` → **184 passed** (165 baseline + 19 новых), 460 assertions, **0 failed**. Багов не найдено — фича ведёт себя точно по контракту (роуты, персист, locked-guard, аддитивный сидер) на всех проверенных сценариях.

Изменённые/новые файлы: `tests/Feature/Access/RolePermissionsManagementTest.php` (новый). Продовый код (`Roles.php`, Blade, роуты, `RolesSeeder.php`) не трогал.
- `.claude/shared/data-contracts.md` не трогал — схема БД/моделей Lead/LeadActivity не менялась, только Livewire-свойства/методы.

## 2026-07-07 — [admin-bi-developer] Kanban-доска Leads — Blade/Alpine/drag-and-drop (вторая половина фичи, backend уже готов от `laravel-fullstack`)

`resources/views/livewire/admin/leads/index.blade.php` — единственный изменённый файл:

- **Toggle таблица/канбан**: сегментированная пара кнопок в шапке страницы (иконки `table-cells` / `view-columns`, heroicons-style inline SVG, `w-4 h-4`, стиль как у остальных иконок файла). Каждая кнопка одновременно: `wire:click="setViewMode('table'|'kanban')"` (реальный серверный переключатель, т.к. `$kanbanColumns` считается только на сервере) и `@click="localStorage.setItem('rsg-admin-leads-view-mode', '...')"` — два независимых обработчика на одном элементе, оба работают штатно (Livewire/Alpine не конфликтуют, паттерн подтверждён прогоном тестов). Активное состояние подсвечивается чисто через Blade-условие (`{{ $viewMode === 'table' ? ... }}`) — без Alpine-реактивности, т.к. `wire:click` и так вызывает full re-render компонента.
- **Персист выбора**: ключ `rsg-admin-leads-view-mode` в `localStorage` (значения `'table'`/`'kanban'`), по образцу `rsg-admin-sidebar-collapsed` из `layouts/admin.blade.php`. Восстановление при загрузке — `x-init` на корневом `<div>` компонента: читает localStorage, и если там `'kanban'`, а серверное состояние ещё `'table'` (сравнение через `@js($viewMode)`), дёргает `$wire.setViewMode('kanban')`. Если сохранено `'table'` — ничего не делает (сервер и так стартует с `table`), лишнего запроса нет.
- **Kanban-доска**: `@if($viewMode === 'kanban')` рендерит 6 колонок в порядке `$kanbanStatuses` (`new/qualified/contacted/in_negotiation/won/lost`, `client` не показывается — колонки для него нет вообще, backend её и не отдаёт). Заголовок колонки — человекочитаемый статус (`match()`, локальный `@php`-блок, цвета через **статическую** карту `textClass` типа `text-primary-700`/`text-success-700`/... — по образцу `x-badge`/`x-lead-status-badge`, специально НЕ через динамическую интерполяцию `text-{{ $color }}-700`, чтобы Tailwind JIT-сканер гарантированно подхватил классы) + счётчик `total`. Карточка лида: имя (кликабельная ссылка на `route('admin.leads.show', $lead)`), компания, телефон, менеджер, дата создания. Футер колонки — «и ещё N», если `remaining > 0`. Доска — `overflow-x-auto` (горизонтальный скролл рядa колонок) + у каждой колонки independent `overflow-y-auto` со `max-height: calc(100vh - 22rem)` (по аналогии с существующим `calc(100vh - 26rem)` у таблицы). Таблица+фильтры+пагинация обёрнуты в `@if($viewMode === 'table')` — рендерятся только в табличном режиме.
- **Drag-and-drop**: чистый HTML5 DnD + Alpine, без сторонних библиотек. `x-data="{ dragging, draggingFrom, overColumn }"` на обёртке доски. Карточка — `draggable="true"`, `@dragstart` кладёт `lead.id` в `dataTransfer` (`setData('text/plain', ...)`) и запоминает `draggingFrom` (статус исходной колонки), `@dragend` сбрасывает состояние. Колонка — `@dragover.prevent` подсвечивает себя (`ring-2 ring-primary-400`) только если это не колонка-источник (`draggingFrom !== status`) — UX-деталь из ТЗ (не подсвечиваем drop в тот же столбец, откуда тащим); `@drop.prevent` читает id из `dataTransfer` и вызывает `$wire.moveLeadStatus(parseInt(id), '<status>')` — тот же вызов пропускается, если `draggingFrom === status` (не даём дропнуть в тот же столбец даже несмотря на то, что backend и так делает no-op).
- **Вызов Livewire-метода из Alpine/JS**: `$wire.setViewMode('table'|'kanban')` и `$wire.moveLeadStatus(parseInt(id), status)` — прямой вызов через магический Livewire 3/4 JS-прокси `$wire.<methodName>(...args)` (multi-arg поддерживается нативно), без `$wire.call(...)`/`Livewire.find(...)` — этот же паттерн уже используется в проекте (`$wire.addProduct(id)` в `documents/quotes/invoices create-form.blade.php`, `$wire.saveField(f)` в `customers/show.blade.php`).
- `php artisan test`: **184 passed**, 460 assertions, 0 failed — регрессий нет (изменения чисто Blade/Alpine, backend `Index.php` не трогал).

## 2026-07-07 — [qa-tester] Feature-тесты на Kanban-доску Leads (`app/Livewire/Admin/Leads/Index.php`)

Новый файл `tests/Feature/Leads/LeadKanbanTest.php` (9 тестов), паттерны/сидинг скопированы из `LeadIndexTest.php`/`LeadEditTest.php`/`LeadConversionTest.php`, отдельный класс — не дублирует существующие хелперы `seedRoles()`/`source()`.

- **Ownership-скоуп канбана** (`set('viewMode', 'kanban')` реально триггерит пересчёт `kanbanColumns()` в `render()`, подтверждено): `sales-manager` видит в канбане только свои лиды (чужой лид другого менеджера не виден ни в одной колонке); `sales-director` и `super-admin` видят лиды всех менеджеров.
- **`client`-лиды исключены из канбана**: лид со `status='client'` не отображается ни в одной из 6 колонок в kanban-режиме, но остаётся видимым в table-режиме при явном `statusFilter='client'` — поведение осознанно разное и последовательное с существующим табличным тестом.
- **`moveLeadStatus()` happy path**: `sales-manager` двигает свой лид `new → qualified` — статус меняется, создаётся `LeadActivity` (`type='status_change'`, `meta=['from'=>'new','to'=>'qualified']`).
- **`moveLeadStatus()` авторизация (deny)**: `sales-manager` не может подвинуть статус чужого лида — `assertForbidden()` (403 из `$this->authorize('update', $lead)`, та же `LeadPolicy`, что и для таблицы/Show), статус лида не меняется, activity не создаётся.
- **`moveLeadStatus()` guard на `client`-статус, невалидный target-статус, и no-op при том же статусе** — все три сценария подтверждены как тихий no-op (без exception, без изменения статуса, без новой `LeadActivity`), включая проверку `count()` до/после для сценария "тот же статус", как отдельно указано в задаче.

**Результат**: `php artisan test` (полный прогон) → **193 passed** (184 baseline + 9 новых), 484 assertions, **0 failed**. Багов не найдено — реализация канбана и `moveLeadStatus()` в `Index.php` полностью соответствует задокументированному контракту (ownership-scope, exclusion `client`-статуса, whitelist статусов, no-op-семантика, authorization).

Изменённые/новые файлы: `tests/Feature/Leads/LeadKanbanTest.php` (новый). Продовый код (`Index.php`, Blade, `LeadPolicy.php`, `CLAUDE.md`) не трогал.
- `.claude/shared/data-contracts.md` не трогал — схема моделей не менялась, только тестовое покрытие существующих Livewire-свойств/методов.
